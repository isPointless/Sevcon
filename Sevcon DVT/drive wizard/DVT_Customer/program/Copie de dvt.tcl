...
set toolname    "C6944 Shiroko Design Verification Test System"
...
proc register {section fname revision} {
    ...
    # revision is in the format $Revision:1.3$
...
proc revision_list {section} {
    ...
    if {![info exists revision_list_[set section]]} {
        ...
        set revs "$section ([total_revision $section]):\n"
        set i 1
        foreach item [lsort [set revision_list_[set section]]] {
...
register DVT "dvt.tcl" {$Revision:1.3$}
...
image create photo .help_gif         -format GIF -data $help_img_data
...
proc genwindow {} {
   ...
   # Help
   button .toolbar.help_button \
       -image .help_gif \
       -command help
   
   set_balloon .toolbar.help_button "Help"

   pack .toolbar.help_button -side left -padx 1
   ...
   label       .can.name -text "CAN" 
   text        .can.text -height 15 -width 60 -yscrollcommand {.can.scry set} -xscrollcommand {.can.scrx set} -wrap none
   ...
   grid configure .can.stats  -column 2 -row 0 -rowspan 3  -sticky nsew
   ...
   set con [text .conarea.txt  -yscrollcommand ".conarea.sb set" -setgrid true -width 80 -height 20]
  ...
  .menubar.can add separator
   ...
   .menubar.vehicle add command -label Interface -command {source veh_if.tcl}
...
proc help { {key " "} } {
   ...
   if {$found == 0} {
      dvtConPuts help "No help on $key.  Type \"help\" for a list of commands"
   }
...
proc loaddll {} {
   if { [catch {load "dvtint.dll" Dvtint} err] } {
...
set helptext(check_can_cli_dump) "Checks the amount of file space used by the CLI/CAN dumps"
set helptextd(check_can_cli_dump) "Returns the number of bytes taken by the CLI/CAN dump files. \
Also prints and error message if more than 100Mb has been used. Use clean_can_cli_dump to delete the files."
...
set helptext(find) "searches available procs for keywords"
set helptexta(find) "f"
set helptextd(find) "allows you to search for a proc you know exists
Usage find <search pattern> <show proc body of hits>
